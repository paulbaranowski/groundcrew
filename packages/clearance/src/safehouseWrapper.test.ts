...
async function runWrapper(input: { args: readonly string[]; tempDir: string }): Promise<StubPaths> {
  ...
  const inheritedPath = readPathEnvironmentVariable() ?? "";
...
function readPathEnvironmentVariable(): string | undefined {
  // oxlint-disable-next-line node/no-process-env -- wrapper integration test must preserve PATH while adding fake binaries.
