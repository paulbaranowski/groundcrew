...
5. Babysit the PR through CI and review feedback. Check the current CI status; if it's still running, wait for it to settle. Auto-fix high-confidence failures (lint, format, typecheck, missing imports) and push the fixes. Reply to any active review-thread comments. Read any CodeRabbit review-body comments and address every actionable item — re-run steps 2–3 (tests + sub-agent review) on each batch of fixes. Run up to 3 babysitting passes total. Wait at least 10 minutes between passes — CodeRabbit posts its review in pieces, so back-to-back passes can land on a partial review and burn a pass that produces nothing. Stop earlier if a pass produces no new fixes or pushes no new commits.
