...
5. Babysit the PR through one CI pass. Check the current CI status; if it's still running, wait for it to settle. Auto-fix high-confidence failures (lint, format, typecheck, missing imports) and push the fixes. Reply to any active review-thread comments. Read any CodeRabbit review-body comments and address every actionable item — re-run steps 2–3 (tests + sub-agent review) on each batch of fixes. Make at most one additional pass if your fixes pushed new commits; otherwise proceed.
