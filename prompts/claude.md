...
5. Invoke the `core:babysit-pr` skill **once** on the PR you just opened. It waits for the current CI pass to settle, auto-fixes high-confidence failures (lint, format, typecheck, missing imports), replies to any active review threads, and summarizes CodeRabbit's review-body comments. Address every issue it surfaces — push fixes back through steps 2–3 (tests + sub-agent review) before the next iteration. Repeat babysit-pr at most one more time only if the first pass pushed commits; otherwise proceed.
